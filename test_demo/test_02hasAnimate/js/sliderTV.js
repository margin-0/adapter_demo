/*! SliderTV | The MIT License (MIT) | Copyright (c) 2016 GibboK */ ! function(t, i, s) {
	...
	e.prototype = {
		init: function(i, s) {
			this.element = i, this.defaults = o, this.options = t.extend({}, o, s), this._getId(), this._listen(this), this._getItemDoms(), this._setItemPositions(), this._calculateMidPoint(), this._setFocus(2), this._createBullets(), this._getBullets(), this._updateBullets(), this._getNavigation(), this._showHidePrevNext()
...
//			}
		...
		getData: function(channelCityId){//console.log(channelCityId)
			...
			var opts = {
				'noMsg': true,
				success: function(data) {//console.log(data);
					var  data = data.data;console.log(data);
					setTimeout(function(){
						$('.out').hide();
						$('#wrapMeng').show();
						$('#wrapMeng_bg').show();
						$('#wrapper-sliderTv').show();
						that.getPointFn();//测试用的
					},2000);
		...
		getShowData: function(data){
			var picStr = '';
			for(var i = 0 ; i < data.length; i++){console.log(i)//console.log(data[i].id)
				picStr += '<div id="item-'+i+'" class="sliderTV__item" data-index="'+data[i].id+'">'
			            +        '<img src="'+data[i].pic+'">'
			            +        '<h1>'+data[i].name+'</h1>'
			            +   '</div>';
			}
			$('#sliderTV').append(picStr);
			$('#sliderTV').sliderTV().init();
		},
...
//		afasdfasdf{{afsadf}}<div>fadfasd{{fasdfsaf}}adfa</div><span>{{afsadfasdf}}</span>
		...
		showDetails: function(data){
			...
			$('.slingerMain .slin_introduce').html(data.description);
